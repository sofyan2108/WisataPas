class FavoritePresenter {
  ...
  async removeFavorite(title) {
    try {
      console.log("Title yang ingin dihapus:", title);
      console.log(
        "Sebelum:",
        this._favorites.map((f) => f.title)
      );
      this._favorites = this._favorites.filter((fav) => fav.title !== title);
      console.log(
        "Sesudah:",
        this._favorites.map((f) => f.title)
      );
      ...
      this._view.showMessage("Item dihapus dari favorit");
    } catch (error) {
      console.error("Error removing favorite:", error);
      this._view.showError("Gagal menghapus dari favorit");
    }
